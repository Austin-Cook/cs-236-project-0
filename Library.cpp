...
void Library::organizeBooksByGenre() {
    for (Book* book : books) {
        ...
        if (it == booksByGenre.end()) {
            ...
            std::vector<Book*> bookList = it->second; //Initiates booklist to vector of books previously under genre
            bookList.push_back(book); //Adds the new book to the genre
			it->second = bookList; //I added this line - Resets the vector of books under genre to the new list with the new book
        }
    }
}
